...
void PatternDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
                            const QModelIndex &index) const
{
    painter->save();

    QImage image = index.data(PatternFrameModel::FrameImage).value<QImage>();
    QImage scaled = image.scaled(getScaledImageSize(option.rect.height(),
...
QSize PatternDelegate::sizeHint(const QStyleOptionViewItem &option,
